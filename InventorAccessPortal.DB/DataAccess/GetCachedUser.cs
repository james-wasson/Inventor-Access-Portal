...
namespace InventorAccessPortal.DB.DataAccess
{
    public static class GetCachedUser
    {
        ...
        public static CachedUser GetNew(Login_datum row)
        {
        ...
        /// <returns>a new cached user object</returns>
        public static CachedUser UserByUsername(String username, DbContext e)
        {
            var row = e.Login_Data.FirstOrDefault(p => p.Username == username);
            return GetNew(row);
        }
        ...
        /// <param name="e">database context</param>
        /// <returns>a new cached user object</returns>
        public static CachedUser UserByEmail(String email, DbContext e)
        {
            var lowerEmail = email.ToLower();
            var row = e.Login_Data.FirstOrDefault(p => p.Investigator.Email_Address.ToLower() == lowerEmail);
            return GetNew(row);
        }
    }
}
